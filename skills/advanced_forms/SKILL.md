...
**Value depends on another field** ("when B changes, set A" — totals, mirroring, clearing a dependent selection): use the form's `addRelation(source, select, onChange)`, in the **constructor body** after `registerFields`, like the mutual pairs above:
...
`select` picks the part of `source`'s value to watch and `onChange` fires only when that part changes, `==`-compared against the last-seen value — so a status-only change on `source` (a gate flip, an error appearing) never fires it, and unlike a raw `addListener` pair a mutual relation pair cannot loop. The form removes the listener in its own `dispose()`; no manual cleanup. A derived field is usually a display field you `markReadOnly()`, and `setValue` on a read-only field is a **no-op**, so pass `force: true` as above. `addRelation` does **not** call `onChange` at registration — only on the next change — so the target still needs the derived value as its own `initialValue:`, otherwise it starts stale (and, seeded after `registerFields`, reports `wasModified: true` before the user touches anything). Throws a `StateError` if the form or `source` is already disposed. A *destructive* relation — `city.select(null)` when `country` changes — is safe here: the `setAutovalidate` sweep inside `validate()` changes no value, so it cannot wipe the user's choice. Attaching or detaching subforms and calling `setValidationEnabled` from `onChange` is supported too. Raw `addListener` + `setValue` is still the right tool only when the target is not a form field at all — a plain `ValueNotifier`, a service callback — and there you do the `==` comparison yourself, because `addListener` fires on **any** state change of the source.
...
`validateAll` is implemented by calling `validateWithAutovalidate()` on every value change, and that method is public: it re-runs the **sync** validator on every leaf field in the tree whose gate is open — recursing into subforms, including read-only fields, and never touching async validators — the tree-wide counterpart to the field-level `revalidateSync()`. Call it by hand when something *other* than a field's own value must force a tree-wide sync revalidation: a locale switch that changes what "too long" means, a feature-flag flip that changes a rule. No network call is owed, so no async check is started.
...
**Migrating a 0.1.x form.** This skill describes building a form against the current API. A port of existing `FieldCubit`/`FormGroupCubit` code to 0.2.0 is a one-time rewrite, not that task: follow `MIGRATION.md` in the package root (with `CHANGELOG.md` for the version history) and use this file only as the reference for what the ported code should end up looking like.
