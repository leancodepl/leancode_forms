...
AdvancedFormScope<SignupFormController>(
  create: (context) => SignupFormController(),
  child: const SignupForm(),
);
```

Already using the `provider` package? `ChangeNotifierProvider<T>` works exactly the same way — use that instead if you'd rather not add another widget to learn.
