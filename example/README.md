...
| [`step_form.dart`](lib/screens/step_form.dart) | A wizard: one subform per step, `await step.validate()` on Next, the parent's `validate()` on Submit, and a per-step `setValidationMode` after the first failure | [README — Subforms](../README.md#subforms) |
